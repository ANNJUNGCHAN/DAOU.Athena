...
test('ensureLocalBearerToken: 설치본처럼 .env가 없으면 토큰을 만들고 저장한다', () => {
  const prev = process.env.ATHENA_LOCAL_BEARER_TOKEN;
  delete process.env.ATHENA_LOCAL_BEARER_TOKEN;
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'athena-local-bearer-'));
  const storePath = path.join(dir, 'local-bearer');
  const { ensureLocalBearerToken } = require('./backend-launcher');
  try {
    const token = ensureLocalBearerToken({
      backendDir: dir,
      storePath,
      readEnvFile: false,
    ...
    assert.equal(fs.readFileSync(storePath, 'utf8').trim(), token);
    assert.equal(process.env.ATHENA_LOCAL_BEARER_TOKEN, token);
    assert.equal(fs.existsSync(path.join(dir, '.env')), false);
    const reused = ensureLocalBearerToken({
      backendDir: dir,
      storePath,
      readEnvFile: false,
      randomBytes: () => { throw new Error('should not regenerate'); },
    });
...
test('설치본은 배포되지 않은 .env를 읽지 않는다', () => {
  const prev = process.env.ATHENA_LOCAL_BEARER_TOKEN;
  delete process.env.ATHENA_LOCAL_BEARER_TOKEN;
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'athena-skip-env-'));
  const storePath = path.join(dir, 'local-bearer');
  fs.writeFileSync(path.join(dir, '.env'), 'ATHENA_LOCAL_BEARER_TOKEN=must-not-read\n');
  const { readLocalBearerToken, ensureLocalBearerToken } = require('./backend-launcher');
  try {
    assert.equal(readLocalBearerToken(dir, { readEnvFile: false }), null);
    const token = ensureLocalBearerToken({
      backendDir: dir,
      storePath,
      readEnvFile: false,
    ...
    assert.notEqual(token, 'must-not-read');
  } finally {
    if (prev == null) delete process.env.ATHENA_LOCAL_BEARER_TOKEN;
    else process.env.ATHENA_LOCAL_BEARER_TOKEN = prev;
    fs.rmSync(dir, { recursive: true, force: true });
...
test('프론트가 계좌를 지정하면 백엔드 .env에 로컬 베어러를 붙인다', () => {
  const prev = process.env.ATHENA_LOCAL_BEARER_TOKEN;
  delete process.env.ATHENA_LOCAL_BEARER_TOKEN;
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'athena-attach-env-'));
  const storePath = path.join(dir, 'local-bearer');
  fs.writeFileSync(path.join(dir, '.env'), 'FOO=keep\n');
  const { ensureLocalBearerToken } = require('./backend-launcher');
  try {
    const token = ensureLocalBearerToken({
      backendDir: dir,
    ...
    const attached = fs.readFileSync(path.join(dir, '.env'), 'utf8');
    assert.match(attached, /^FOO=keep$/m);
    assert.match(attached, new RegExp(`^ATHENA_LOCAL_BEARER_TOKEN=${token}$`, 'm'));
  } finally {
    if (prev == null) delete process.env.ATHENA_LOCAL_BEARER_TOKEN;
    else process.env.ATHENA_LOCAL_BEARER_TOKEN = prev;
    fs.rmSync(dir, { recursive: true, force: true });
...
test('ensureLocalBearerToken: 개발 .env가 있으면 저장 파일을 만들지 않는다', () => {
  const prev = process.env.ATHENA_LOCAL_BEARER_TOKEN;
  delete process.env.ATHENA_LOCAL_BEARER_TOKEN;
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'athena-env-bearer-'));
  const storePath = path.join(dir, 'unused-store');
  fs.writeFileSync(path.join(dir, '.env'), 'ATHENA_LOCAL_BEARER_TOKEN=from-dev-env\n');
  const { ensureLocalBearerToken } = require('./backend-launcher');
  try {
    const token = ensureLocalBearerToken({
      backendDir: dir,
      storePath,
      readEnvFile: true,
      randomBytes: () => { throw new Error('should not generate'); },
    });
    assert.equal(token, 'from-dev-env');
    assert.equal(fs.existsSync(storePath), false);
    assert.equal(process.env.ATHENA_LOCAL_BEARER_TOKEN, 'from-dev-env');
  } finally {
    if (prev == null) delete process.env.ATHENA_LOCAL_BEARER_TOKEN;
    else process.env.ATHENA_LOCAL_BEARER_TOKEN = prev;
    fs.rmSync(dir, { recursive: true, force: true });
...
test('설치본 main은 계좌 저장 때 백엔드 설정 파일을 붙인다', () => {
  const main = fs.readFileSync(path.join(__dirname, '../../main.js'), 'utf8');
  assert.match(main, /ensureLocalBearerToken\(/);
  assert.match(main, /attachSettings:\s*true/);
  assert.match(main, /readEnvFile:\s*!backendLauncher\.packagedBackendLayout\(\)/);
  assert.doesNotMatch(main, /backendLauncher\.readLocalBearerToken\(/);
});
