...
function extractOrderConfirmationNotice(content) {
  ...
  const created = parsed.order_ticket_created === true;
  if (!created && parsed.order_ticket_created !== false) return null;
  const draft = parsed.order_draft;
  if (created && (!draft || typeof draft !== 'object' || Array.isArray(draft))) return null;
  ...
  return {
    ...
    operationRef: typeof parsed.operation_ref === 'string' ? parsed.operation_ref : null,
    orderDraft: created ? draft : null,
  };
}
