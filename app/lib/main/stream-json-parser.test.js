...
test('classifyCanvasBlock: 티켓 미생성 주문 확인 결과만 전용 종결 상태로 분류한다', () => {
  ...
  assert.deepEqual(result, {
    toolUseId: 'gold-order', status: 'needs_confirmation', message,
    ticketCreated: false, operationRef: null, orderDraft: null,
  });
...
test('classifyCanvasBlock: 초안이 있는 주문 확인 결과는 티켓 생성으로 분류한다', () => {
  const message = '주문 티켓을 열었습니다. 앱에서 내용을 확인한 뒤 전송하세요.';
  const result = classifyCanvasBlock({
    toolUseId: 'samsung-buy', isError: false, meta: null,
    content: JSON.stringify({
      status: 'needs_confirmation', confirmation_required: true,
      order_ticket_created: true, order_submitted: false, message,
      operation_ref: 'base:kt10000',
      order_draft: { dmst_stex_tp: 'KRX', stk_cd: '005930', ord_qty: '2', trde_tp: '3', side: 'buy' },
    }),
  });
  assert.equal(result.status, 'needs_confirmation');
  assert.equal(result.ticketCreated, true);
  assert.equal(result.operationRef, 'base:kt10000');
  assert.equal(result.orderDraft.stk_cd, '005930');
