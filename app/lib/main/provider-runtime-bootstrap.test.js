...
test('main account, cold-mutation, shutdown, and history commit boundaries are fail closed', () => {
  ...
  assert.match(source, /result\.status === 'needs_confirmation'[\s\S]*?presentProviderOrderTicket\([\s\S]*?emitProviderOrderDraft\(context\.conversationId, payload\)[\s\S]*?persistentTerminalAnswers\.set\(context\.conversationId, result\.message\)[\s\S]*?return;/);
  assert.match(source, /r\.status === 'needs_confirmation'[\s\S]*?presentProviderOrderTicket\([\s\S]*?emitProviderOrderDraft\(turnConversationId, payload\)/);
...
test('주문 티켓 미생성 결과는 persistent 이력과 cold 최종문을 대체하고 다음 턴에는 남지 않는다', () => {
  ...
  vm.runInContext(`
    ...
    const presentProviderOrderTicket = () => false;
...
test('main MCP list is read-only and conversation rotation uses the shared provider tail', () => {
  ...
  // 프로바이더 런타임이 켜진 경로에서는 목록 조회가 읽기 전용이어야 한다. 꺼진
  // 기본 경로는 병합 전 main 그대로 평문 환경값을 옮기므로, 그 호출이 반드시
  // !providerRuntimeEnabled 가드 안에만 있는지를 본다.
  assert.match(listHandler, /if \(!providerRuntimeEnabled\) \{[\s\S]*?migratePlaintextEnv/);
  assert.doesNotMatch(listHandler.slice(listHandler.indexOf('  return mcpCli.list()')), /migratePlaintextEnv/);
