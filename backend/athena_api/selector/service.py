...
_ORDER_TICKET_FIELDS = frozenset(
    {
        "dmst_stex_tp",
        "stk_cd",
        "ord_qty",
...
def _order_ticket_details(plan: VerifiedPlan) -> dict[str, Any]:
    arguments = plan.arguments if isinstance(plan.arguments, dict) else {}
    draft = {
        key: value
        for key, value in arguments.items()
        if key in _ORDER_TICKET_FIELDS and value is not None
    }
    side = _CASH_ORDER_SIDES.get(plan.operation_ref)
    if side is not None:
        draft["side"] = side
    return {
        ...
        *,
        ...
        if query_only and document.kind == "order":
            raise OrderTicketRequiredError(
                ...
                details=_order_ticket_details(plan),
            )
