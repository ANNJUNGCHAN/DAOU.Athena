...
async def test_render_with_plan_opens_order_ticket_from_signed_draft(
    mock_http_client,
):
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
    ...
    assert result.isError is False
    assert result.structuredContent["order_ticket_created"] is True
    assert result.structuredContent["order_submitted"] is False
    assert result.structuredContent["operation_ref"] == "base:kt10000"
    assert result.structuredContent["order_draft"]["stk_cd"] == "005930"
    assert "guarded-order-token" not in result.content[0].text
