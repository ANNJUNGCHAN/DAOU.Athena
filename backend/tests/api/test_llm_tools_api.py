...
def test_query_only_call_rejects_order_before_consuming_or_dispatching() -> None:
    ...
    body = rejected.json()
    assert body["detail"] == "Selector request failed"
    assert body["code"] == "ORDER_TICKET_REQUIRED"
    assert body["message"] == "Order plans must be submitted through the confirmed order ticket"
    assert body["details"]["operation_ref"] == "base:kt10000"
    assert body["details"]["order_draft"]["stk_cd"] == "005930"
    assert body["details"]["order_draft"]["ord_qty"] == "1"
    assert body["details"]["order_draft"]["side"] == "buy"
    assert body["details"]["next_actions"] == ["open_order_ticket"]
    assert order.calls == []
