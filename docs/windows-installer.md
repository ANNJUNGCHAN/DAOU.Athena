...
Athena가 사용하는 Claude, Codex, Grok 같은 외부 AI 공급자 CLI와 로그인 상태는 설치 파일에 포함되지 않습니다. 이미 설치된 CLI는 앱이 현재 사용자 환경에서 탐색합니다. CLI가 없는 경우에만 별도 설치가 필요하며, 연결에는 해당 공급자의 로그인 상태가 필요합니다. 설치 파일과 빌드 자료에는 실제 계정, API 키, `.env`, 로컬 사용자 설정을 넣지 않습니다. 설치된 앱은 배포되지 않은 `.env`를 읽지 않습니다. 설정 화면에서 APP KEY / SECRET KEY를 저장할 때 앱이 백엔드 쪽에 로컬 설정 파일을 붙입니다.
